...
$selectDuration = "SELECT sdate, edate FROM info WHERE name = 'markingperiod'";
...
if(isset($_POST['btn_login'])){
	...
	$enter_password = MD5($_POST['password']);
		...
		$select1 = "SELECT * FROM chiefdetails WHERE year1='$cdyear' AND nic='$username' AND password='$enter_password'";
		$result1 = $conn->query($select1);
		if ($result1->num_rows > 0) {
			
			while($rows1 = $result1->fetch_assoc()){
				$ukey= $rows1['id']; 
				$panelno= $rows1['panelno'];  
				$password= $rows1['password'];				
			}
			
			$_SESSION['login_user'] = $username;
			$_SESSION['user_keye'] = $ukey;
			$_SESSION['panel_no'] = $panelno;

			if($password=="07811dc6c422334ce36a09ff5cd6fe71"){  //2024
				echo "<script>
						window.location.href='nwpassword.php';
					</script>";
				session_register("login_user","user_keye","panel_no");
			}
			else{
				echo "<script>
