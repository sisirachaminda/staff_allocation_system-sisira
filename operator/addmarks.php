...
while($row10 = $result10->fetch_assoc()) {
	...
	$mrksch10=$row10['servicegraduate_benchmark_year'];
	$mrksch11=$row10['servicegraduate_benchmark_overmark'];
	
	$mrksch12=$row10['period_count1'];
	$mrksch13=$row10['period_count1_mark'];
	$mrksch14=$row10['period_count2'];
	$mrksch15=$row10['period_count2_mark'];
	$mrksch16=$row10['period_count3'];
	$mrksch17=$row10['period_count3_mark'];
	
	$mrksch18=$row10['passpresenatage_benchmark1'];
	$mrksch19=$row10['passpresenatage_mark1'];
	$mrksch20=$row10['passpresenatage_benchmark2'];
	$mrksch21=$row10['passpresenatage_mark2'];
	$mrksch22=$row10['passpresenatage_benchmark3'];
...
if(isset($_POST['runProcess'])) {
	...
	while($row2 = $result2->fetch_assoc()) {
		...
		if($subCount>= 1 && $mrksch12<= 10) {
			$mark8 = $mrksch13;
		} else if($subCount > $mrksch12 && $subCount <= $mrksch14) {
			$mark8 = $mrksch15;
		} else if($subCount > $mrksch14 && $subCount <= $mrksch16) {
			$mark8 = $mrksch17;
		} else {
			$mark8 = 0;
		}
		...
		if($passPercentage >= $mrksch18 && $passPercentage < $mrksch20) {
			$mark9 = $mrksch19;
		} else if($passPercentage >= $mrksch20 && $passPercentage < $mrksch22) {
			$mark9 = $mrksch21;
		} else if($passPercentage >= $mrksch22) {
			$mark9 = $mrksch23;
		} else {
			$mark9 = 0;
		}
		...
		while($rowcenter1 = $resultcenter1->fetch_assoc()) {
				...
				if($numofCand1 >= $isExaminer1) {
					...
					$select11 = "SELECT * FROM basicdetails INNER JOIN experience_selectsubject ON basicdetails.examiner_id=experience_selectsubject.examiner_id 
																								...
																								AND basicdetails.principleApproveStatus=1
																								AND basicdetails.appointedAs IS NULL														
																								ORDER BY experience_selectsubject.assi_experince DESC, basicdetails.marks DESC LIMIT $numofCand1";
