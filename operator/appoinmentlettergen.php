...
if(isset($_POST['runProcess'])){
	$select5= "SELECT * FROM basicdetails WHERE year1='$cdyear' 
											AND principleApproveStatus=1 
											AND appointedAs IS NOT NULL
											 AND appoinmentletterdte IS NULL";
	$result5 = $conn->query($select5);
	while($row5 = $result5->fetch_assoc()) {
		if ($stmt = $conn->prepare("UPDATE basicdetails SET appoinmentletterdte='$cddate' WHERE year1='$cdyear' 
																							AND principleApproveStatus=1 
																							AND appointedAs IS NOT NULL
																							AND appoinmentletterdte IS NULL")) {
			$stmt->execute();
	...
	$select6= "SELECT * FROM chiefdetails WHERE year1='$cdyear' 
											AND appoinmentletterdte IS NULL";
	$result6 = $conn->query($select6);
	while($row6 = $result6->fetch_assoc()) {
		if ($stmt1 = $conn->prepare("UPDATE chiefdetails SET appoinmentletterdte='$cddate' WHERE year1='$cdyear' 
																							AND appoinmentletterdte IS NULL")) {
			$stmt1->execute();
            ...
            <form method="post" enctype="multipart/form-data"> 
				<div class='row mt-4'>
					<div class='col col-sm-12 col-lg-5'>
						<input type='submit' name='runProcess' value='Generate Appoinment Letter' class='btn btn-success'>
					</div>
										...
										<tbody>
											...
											while($row1 = $result1->fetch_assoc()) {
													...
													<tr>
														...
														<td width='5%'><a href='".$letterlink."'><button class='btn btn-primary' type='button'>View</button></a></td>
													</tr>";
										...
										<tbody>
											...
											$select7 = "SELECT * FROM chiefdetails INNER JOIN markingcenters ON  chiefdetails.panelNo=markingcenters.panelNo
																					WHERE chiefdetails.year1='$cdyear' 
																					  AND chiefdetails.appoinmentletterdte IS NOT NULL";
											...
											while($row7 = $result7->fetch_assoc()) {
													...
													while($row10 = $result10->fetch_assoc()) {
														  $subject1=$row10['subject_name'];
													}
													
													
													$letterlink1="fpdf/chiefexaminer_appoinment_letter.php?id=".$row7['id']."";
													
													
													echo "
													<tr>
														<td width='10%'>".$row7['panelNo']."</td>
														<td width='10%'>".$school1."</td>
														<td width='10%'>".$town1."</td>
														<td width='10%'>".$row7['subjectID']." ".$subject1."</td>
														<td width='10%'>".$medium1."</td>
														<td width='10%'>".$row7['appoinmentletterdte']."</td>
														<td width='25%'>".$row7['initialName']."</td>
														<td width='10%'>".$row7['nic']."</td>
														<td width='5%'><a href='".$letterlink1."'><button class='btn btn-primary' type='button'>View</button></a></td>
													</tr>";
