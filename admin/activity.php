...
if(isset($_POST['btn_add'])){
	$nr1=$_POST['activityname'];
	
	
	$select = "SELECT * FROM activity WHERE activity_name='$nr1'";
	$result = $conn->query($select);	
	if ($result->num_rows == 0) {
		
		if ($stmt = $conn->prepare("INSERT INTO activity(activity_name) 
												VALUES ('$nr1')")) {
			$stmt->execute();
			echo "<script>
		...
		echo "<script>
				alert('Sorry ! Duplicate Activity !')
				window.location.href = 'activity.php';
		</script>";
	}
}
...
if(isset($_GET['del'])){
		if ($stmt = $conn->prepare("DELETE FROM activity WHERE activity_id='$_GET[del]'")) {
			$stmt->execute();
			echo "<script>
            ...
            <fieldset>
              <div class="form-row">
				...
				<div class="col-md-12 mb-3">
                  <label for="validationTooltip01">Activity</label>
                  <input type="text" class="form-control" name="activityname" placeholder="Activity" required>
                </div>
			  ...
			  <div class="form-row">
				...
				<div class="col-md-4 mb-3">
					<button type="submit" class="btn btn-success" name="btn_add">Add Activity</button>  
				</div>
			   </div>
            </fieldset>
			</form>  
                          ...
                          <?php 
                            ...
                            if ($result2->num_rows > 0) {
								...
								while($row2 = $result2->fetch_assoc()) {
                                  ...
                                  echo "<tr>
										  <td scope='row' align='left'>".$row2['activity_name']."</td>
										  <td align='left'><a href='activity.php?del=".$row2['activity_id']."'><button class='btn btn-danger'>Delete</button> </a></td>
                                        </tr>";
                                }
                            } 
							else {
								echo "<tr><td colspan='6' align='center'>No Activity</td></tr>";
                            }
                          ?>
                        </tbody>
                      </table>
