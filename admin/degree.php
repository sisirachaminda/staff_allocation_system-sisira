...
if(isset($_POST['btn_add'])){
	$nr1=$_POST['degreename'];
	
	
	$select = "SELECT * FROM degree WHERE degree_name='$nr1'";
	$result = $conn->query($select);	
	if ($result->num_rows == 0) {
		
		if ($stmt = $conn->prepare("INSERT INTO degree(degree_name) 
												VALUES ('$nr1')")) {
			$stmt->execute();
			echo "<script>
...
if(isset($_GET['del'])){
		if ($stmt = $conn->prepare("DELETE FROM degree WHERE degree_id='$_GET[del]'")) {
			$stmt->execute();
			echo "<script>
            ...
            <fieldset>
              <div class="form-row">
				...
				<div class="col-md-12 mb-3">
                  ...
                  <input type="text" class="form-control" name="degreename" placeholder="Degree Name" required>
                </div>
			  ...
			  <div class="form-row">
				...
				<div class="col-md-4 mb-3">
					<button type="submit" class="btn btn-success" name="btn_add">Add Degree</button>  
				</div>
			   </div>
            </fieldset>
			</form>  
                          ...
                          <?php 
                            ...
                            if ($result2->num_rows > 0) {
								...
								while($row2 = $result2->fetch_assoc()) {
                                  ...
                                  echo "<tr>
										  <td scope='row' align='left'>".$row2['degree_name']."</td>
										  <td align='left'><a href='degree.php?del=".$row2['degree_id']."'><button class='btn btn-danger'>Delete</button> </a></td>
                                        </tr>";
                                }
                            } 
							else {
								echo "<tr><td colspan='6' align='center'>No Degree</td></tr>";
                            }
                          ?>
                        </tbody>
                      </table>
