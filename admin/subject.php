...
if(isset($_POST['btn_add'])){
	$nr1=$_POST['subjectid'];
	$nr2=$_POST['subjectname'];
	
	$select = "SELECT * FROM subjects WHERE subject_id='$nr1' OR subject_name='$nr2'";
	$result = $conn->query($select);	
	if ($result->num_rows == 0) {
		
		if ($stmt = $conn->prepare("INSERT INTO subjects (subject_id,
													    subject_name) 
												VALUES ('$nr1',
														'$nr2')")) {
			$stmt->execute();
		...
		echo "<script>
				alert('Sorry ! Duplicate subjects !')
				window.location.href = 'subject.php';
		</script>";
	}
}
...
if(isset($_GET['del'])){
		if ($stmt = $conn->prepare("DELETE FROM subjects WHERE id='$_GET[del]'")) {
			$stmt->execute();
			echo "<script>
            ...
            <fieldset>
              <div class="form-row">
                <div class="col-md-6 mb-3">
                  <label for="validationTooltip01">Subject ID</label>
                  <input type="text" class="form-control" name="subjectid" placeholder="Subject ID" required>
                </div>
				
				<div class="col-md-6 mb-3">
                  <label for="validationTooltip01">Subject Name</label>
			  ...
			  <div class="form-row">
				...
				<div class="col-md-4 mb-3">
					<button type="submit" class="btn btn-success" name="btn_add">Add Subjects</button>  
				</div>
			   </div>
            </fieldset>
			</form>  
                          ...
                          <?php 
                            ...
                            if ($result2->num_rows > 0) {
								...
								while($row2 = $result2->fetch_assoc()) {
                                  ...
                                  echo "<tr>
										  <td scope='row' align='left'>".$row2['subject_id']."</td>
										  <td align='left'>".$row2['subject_name']."</td>
										  <td align='left'><a href='subject.php?del=".$row2['id']."'><button class='btn btn-danger'>Delete</button> </a></td>
                                        </tr>";
                                }
                            } 
							else {
								echo "<tr><td colspan='6' align='center'>No Subjects</td></tr>";
                            }
                          ?>
                        </tbody>
                      </table>
